function [ A,B,Q,Init_X,Init_Cov,C,bias,D,R,Theta ] = Maximization_nondiag_bias( Xsmth_t,Covsmth_t,VarW_t,CorW_t,Y_Obs,N_Obs,Previous_Theta,settings )
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This code plays the role of maximization step in the multiscale EM (this function is exactly like Maximization_diag_bias.m, except the R matrix will be nondiag)
...
if settings.switch_biasobs == 1
    ...
    Raux = Sum_VarY - C_bias * transpose(Sum_CorYX_bias) - Sum_CorYX_bias * transpose(C_bias) + C_bias * Sum_VarW_Scale_bias * transpose(C_bias);
...
for i=1:N
    ...
    Theta(:,i)=(fminunc(myf,Previous_Theta(:,i)',options))';
